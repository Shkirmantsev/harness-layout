...
| [Current task](../.ai/state/CURRENT.md) | Generated concise view of the active handoff; read at session intake | Regenerated by `session_state.py`; never edit manually |
| [Task handoffs](../.ai/state/handoffs/) | Canonical structured task context, progress, decisions, file hashes, and verification | Project-visible operational state; commit when cross-machine resume is required |
