---
name: graphify
description: "Build or query a persistent knowledge graph for code, docs, papers, or media. Use when the user explicitly asks for Graphify/knowledge-graph analysis, or when an existing graphify-out graph materially helps an architecture or relationship question. Do not replace exact file, text, or symbol search with Graphify."
---
...
```bash
# Detect an existing installation. Installing software is a separate, permissioned action.
PYTHON=""
GRAPHIFY_BIN=$(command -v graphify 2>/dev/null)
# Read a safe interpreter path from the graphify entry point.
...
if [ -z "$PYTHON" ] && python3 -c "import graphify" 2>/dev/null; then
    PYTHON="python3"
fi
if [ -z "$PYTHON" ]; then exit 42; fi
...
If detection exits 42, stop and ask whether the user wants to install the `graphifyy` package. Do not run `uv tool install`, `pip install`, or another networked installer without explicit permission. If the import succeeds, move to Step 2.
